...
class CustomTextField: UITextField {
    ...
    var finalStringWithoutFormatting : String {
        return CustomTextField.makeOnlyDigitsString(_textWithoutSecureBullets)
    }
    ...
    fileprivate func registerForNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(CustomTextField.textDidChange), name: NSNotification.Name(rawValue: "UITextFieldTextDidChangeNotification"), object: self)
    }
    ...
    func textDidChange() {
        ...
        if formatting != .noFormatting && currentTextForFormatting.characters.count > 0 && formattingPattern.characters.count > 0 {
            let tempString = CustomTextField.makeOnlyDigitsString(currentTextForFormatting)
